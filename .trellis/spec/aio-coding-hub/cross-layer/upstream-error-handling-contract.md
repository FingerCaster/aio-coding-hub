...
- The settings UI exposes one `Upstream Error Handling` section with segmented
  `Retry Rules` and `Final Response Rewrite` modes. Retry edits persist only
  `upstream_retry_policy`; rewrite edits persist only
  `upstream_error_response_rules`. A rule never combines retry and rewrite
  actions.
...
- Before downstream commit, a retry-keyword match may discard the buffered
  ...
  over non-retry keywords; matching is case-insensitive literal text. At guard
  expiry or buffer cap, or after downstream commit, preserve the original SSE
  without splicing output from another attempt. Buffer-cap release is
...
- If all Providers fail on retryable pre-commit stream errors, return the
  ...
  original HTTP 200 capacity stream.
...
- Rewrite matching supports priority, Any/All, status codes,
  ...
  safe `Retry-After` and `x-trace-id` headers.
...
| All Providers end in retryable capacity streams | Return standard 502 / fake-200 envelope with bounded evidence |
...
- Native Codex route tests cover exact path/bridge gating, event and data types,
  ...
  pre-commit retry success, all-Provider failure, guard expiry, 1 MiB cap, and
...
- Frontend tests cover segmented mode switching, retry/rewrite save isolation,
  dialog CRUD, longest legal text, mobile-width containment, and log-hit badges.
...
// Correct: one product section, separate schemas and save ownership.
persist({ upstream_retry_policy: retryPolicy });
