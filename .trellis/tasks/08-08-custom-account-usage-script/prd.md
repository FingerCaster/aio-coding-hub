...
- [ ] 用户确认后，自定义适配器能完成一次受限请求并显示归一化余额、套餐剩余、周期额度或过期状态。
- [ ] 脚本语法/运行错误、死循环、原生内建阻塞、子进程异常、输出超限和并发超限均在有界时间内失败；终止与回收本身也有硬上限且不遗留子进程。
- [ ] HTTP、Origin、重定向、Cookie/Set-Cookie/Header、请求/响应大小、占位符精确替换、JSON 和状态映射的正反路径均有合成测试。
- [ ] 测试或启用前取消原生确认不会加载凭据；确认期间配置变化会拒绝陈旧执行。
- [ ] 脚本或 Origin 变化撤销旧确认；仅 API Key 轮换仍使用当前密钥和原授权目标。
- [ ] 分享、完整导出、备份和导入不包含脚本、额外 Origin 或授权材料，也不会激活自定义适配器。
- [ ] 手工构造的导入载荷无法保留自定义源码或 Origin；导入后必须从无脚本的禁用状态重新配置。
- [ ] 本机复制保留自定义脚本草稿但不继承授权；源 custom 已启用时只有新确认成功才创建副本，取消为零写入，源 custom 未启用时副本保持禁用。
- [ ] 桌面初始、定时和手动读取同一 Provider 时共享后端请求和结果；in-flight 强刷产生至多一次尾随刷新，旧 generation 结果不能覆盖新配置。
- [ ] 四个 Provider 并发上限不会形成无界等待任务；后端快照只返回当前 generation/配置指纹的归一化结果，不存在、失效或锁不可读时不返回陈旧快照。
- [ ] 秒内连续保存、API Key 轮换、enable/disable 和完整导入复用 Provider ID 都能拒绝旧完成；name/note 与 display timed 开关变化不误清有效结果。
- [ ] 现有 sub2api、NewAPI、手动刷新、缓存清理和展示测试保持通过。
- [ ] 生成绑定由 Rust 重建，前端类型、日志脱敏、Rust 测试、TypeScript、lint、格式和敏感信息审计通过。
