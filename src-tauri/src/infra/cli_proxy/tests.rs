...
#[test]
fn claude_proxy_sync_restores_refreshed_backup_when_apply_fails() {
    let test_app = CliProxyTestApp::new();
    let handle = test_app.handle();
    let base_origin = "http://127.0.0.1:37123";
    let settings_path = enable_claude_proxy_over_direct_config(
    ...
    restore_enabled_keep_state(&handle).expect("restore direct state");
    let root = cli_proxy_root_dir(&handle, "claude").expect("proxy root");
    let backup_path = cli_proxy_files_dir(&root).join("settings.json");
    let manifest_path = cli_proxy_manifest_path(&root);
    let backup_before = std::fs::read(&backup_path).expect("original backup");
    let manifest_before = std::fs::read(&manifest_path).expect("original manifest");
    let invalid_direct = br#"{"env":false,"user_edit":"preserve-me"}"#;
    std::fs::write(&settings_path, invalid_direct).expect("write invalid direct settings");
    ...
    let result = synced
        ...
        .find(|result| result.cli_key == "claude")
        .unwrap();
    assert!(!result.ok, "invalid env must fail application");
    assert_eq!(result.error_code.as_deref(), Some("CLI_PROXY_SYNC_FAILED"));
    assert_eq!(std::fs::read(&settings_path).unwrap(), invalid_direct);
