    ...
    async fn client_build_error_is_persisted_for_each_provider() {
        ...
        let provider_ids = {
            ...
            for (id, name) in [(41_i64, "oauth-a"), (42_i64, "oauth-b")] {
                conn.execute(
                    "INSERT INTO providers(id, provider_uuid, cli_key, name, base_url, api_key_plaintext, created_at, updated_at) VALUES (?1, ?3, 'codex', ?2, '', '', 1, 1)",
                    rusqlite::params![id, name, crate::shared::uuid::new_uuid_v4()],
                )
