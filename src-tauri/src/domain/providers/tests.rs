...
use super::*;
use crate::domain::sort_modes::create_mode;
...
struct ProviderPersistenceSnapshot {
    ...
    default_route_order: Vec<(String, i64)>,
    sort_mode_order: Vec<(i64, String, i64, i64)>,
}
...
"#,
    ...
    let sort_mode_order = {
        let mut statement = conn
            .prepare(
                r#"
SELECT mode_id, cli_key, sort_order, enabled
...
fn seed_sort_mode_provider_order(
    ...
    let conn = db.open_connection().expect("open db");
    for (sort_order, (provider_id, enabled)) in providers.iter().enumerate() {
        conn.execute(
            r#"
INSERT INTO sort_mode_providers(
  mode_id, cli_key, provider_id, sort_order, enabled, created_at, updated_at
...
fn upsert_confirmed_custom_account_usage(
    db: &crate::db::Db,
    name: &str,
    base_url: &str,
) -> ProviderSummary {
    ...
    super::queries::upsert_with_provider_uuid(db, params, Some(provider_uuid))
        .expect("save confirmed custom account usage")
}
...
fn delete_cascades_all_route_references_and_preserves_similar_provider_state() {
    ...
    seed_sort_mode_provider_order(
        ...
        primary_mode.id,
        "claude",
        &[(target.id, true), (retained.id, false)],
    );
    ...
    seed_sort_mode_provider_order(
        ...
        secondary_mode.id,
        "claude",
        &[(retained.id, true), (target.id, false)],
    );
    ...
    assert_eq!(
        ...
        vec![("claude".to_string(), 0)]
    );
    assert_eq!(
        retained_before.sort_mode_order,
        vec![
            (primary_mode.id, "claude".to_string(), 1, 0),
            (secondary_mode.id, "claude".to_string(), 0, 1),
        ]
    );
    ...
    assert_eq!(
        ...
        retained_before,
        "deleting by stable provider ID must not reorder or toggle a similar provider"
    );
...
fn provider_delete_is_blocked_while_managed_profile_references_its_model() {
    ...
    seed_sort_mode_provider_order(
        ...
        mode.id,
        "codex",
        &[(provider.id, true), (retained.id, false)],
    );
...
fn custom_account_usage_config_round_trips_only_in_local_provider_values() {
    ...
    let saved = super::queries::upsert_with_provider_uuid(&db, params, Some(provider_uuid))
        .expect("save custom account usage config");
...
fn upsert_without_extension_values_persistently_revokes_changed_base_origin_permission() {
    ...
    let saved = upsert_confirmed_custom_account_usage(&db, provider_name, origin_one_url);
...
fn upsert_without_extension_values_keeps_permission_for_same_origin_and_key_rotation() {
    ...
    let saved = upsert_confirmed_custom_account_usage(
        &db,
        provider_name,
        "https://same-origin.example.test/v1",
    );
    let saved_values = account_usage_values(&saved);
