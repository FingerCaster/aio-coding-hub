...
fn normalize_concrete_model_id(value: &str) -> Result<String, String> {
...
fn parse_model_catalog_with_format(
    ...
    for item in items {
        ...
        let normalized =
            normalize_concrete_model_id(raw).map_err(|_| DiscoveryParseError::InvalidResponse)?;
...
fn build_discovery_client() -> Result<reqwest::Client, String> {
...
pub(crate) async fn provider_models_discover<R: tauri::Runtime>(
    ...
    if providers::has_bridged_input_semantics(
        input.source_provider_id,
        input.bridge_type.as_deref(),
    ) {
        ...
        let lookup = blocking::run("provider_models_discover_oauth_provider", {
            ...
            move || -> crate::shared::error::AppResult<providers::ProviderOAuthDetails> {
                ...
                if provider.cli_key != cli_key
                    ...
                    || providers::has_bridged_input_semantics(
...
mod tests {
    ...
    async fn edit_with_empty_api_key_uses_stored_key_without_request_log() {
        ...
        let app = tauri::test::mock_app();
        let db_state = DbInitState(tokio::sync::Mutex::new(Some(db.clone())));
