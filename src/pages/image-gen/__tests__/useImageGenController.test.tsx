...
describe("pages/image-gen/useImageGenController", () => {
  ...
  it("downloads a generated memory image through the backend-owned save dialog", async () => {
  ...
  it("aborts download when the save dialog is cancelled", async () => {
    vi.mocked(imageGenSaveImage).mockResolvedValue(false);
    const { result } = await renderController();
    await act(async () => {
      await result.current.downloadImage({
        kind: "memory",
    ...
    expect(imageGenSaveImage).toHaveBeenCalled();
    expect(toast.success).not.toHaveBeenCalledWith("图片已保存");
  });
