...
describe("services/providers/providers", () => {
  ...
  it("device-flow poll and cancel failures never persist the bearer flow id", async () => {
    ...
    vi.mocked(commands.providerOauthPollDeviceFlow).mockResolvedValueOnce({
      status: "error",
      error: { nested: { flow_id: capability } } as unknown as string,
    });
  ...
  it("providerAccountUsageFetch uses generated ipc", async () => {
    vi.mocked(commands.providerAccountUsageSnapshot).mockResolvedValueOnce({
      status: "ok",
      data: {
    ...
    expect(commands.providerAccountUsageSnapshot).toHaveBeenCalledWith(52);
